...
import {
    ...
    isDownloaded as checkIsDownloaded, // Renamed to avoid conflict
    setupDownloadedMusicList,
    useDownloaded as useDownloadedSheetState, // Renamed to avoid conflict
...
class DownloaderClass extends EventEmitter<IDownloaderEvent> { // Renamed class to avoid conflict with exported object
    ...
    constructor() {
        super();
        // Listener for debugging or internal state changes
    ...
    public async download(musicItems: IMusic.IMusicItem | IMusic.IMusicItem[]) {
        ...
        const downloadTasks = itemsToQueue.map((it) => {
            ...
            const task = async () => {
                ...
                if (!currentStatus || (currentStatus.status !== DownloadState.WAITING && currentStatus.status !== DownloadState.ERROR /* Allow retry from error */)) {
        ...
        for (const quality of qualityOrder) {
            try {
                ...
                const taskStatus = this.getTaskStatus(musicItem) || { status: DownloadState.DOWNLOADING };
                 this.setTaskStatus(musicItem, { // Update status with mediaSource and realQuality
                    ...taskStatus,
    ...
    public setConcurrency(concurrency: number) {
        if (this.downloadTaskQueue) {
            this.downloadTaskQueue.concurrency = Math.max(1, Math.min(
                concurrency,
                DownloaderClass.ConcurrencyLimit // Use static property
            ));
        }
    }
    ...
    public useDownloadTaskStatus(musicItem: IMusic.IMusicItem | null) {
      ...
      reactUseEffect(() => {
        ...
        setStatus(this.getTaskStatus(musicItem)); // Set initial status
...
const downloaderInstance = new DownloaderClass();

// Define the useDownloadState hook
function useDownloadStateHook(musicItem: IMusic.IMusicItem | null): DownloadState {
    ...
    const downloaded = useDownloadedSheetState(musicItem); // from './downloaded-sheet'

    if (downloaded) { // useDownloaded returns boolean
...
export default {
    ...
    download: (items: IMusic.IMusicItem | IMusic.IMusicItem[]) => downloaderInstance.download(items),
    setConcurrency: (concurrency: number) => downloaderInstance.setConcurrency(concurrency),
    useDownloadTaskStatus: (item: IMusic.IMusicItem | null) => downloaderInstance.useDownloadTaskStatus(item),
