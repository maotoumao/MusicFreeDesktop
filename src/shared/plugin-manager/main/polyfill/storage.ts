...
export default function createPluginStorage(scope: string) {
    ...
    async function loadStorage() {
        ...
        try {
            const storagePath = path.resolve(app.getPath("appData"), "./musicfree-plugin-storage/" + CryptoJS.MD5(scope).toString() + ".json");
