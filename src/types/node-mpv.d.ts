...
declare module 'node-mpv' {
  export interface MpvOptions {
    ...
    audio_only?: boolean;
    time_update?: number;
    // observeAllProperties?: boolean; // 这个选项在 jeffvli 的 fork 中似乎没有，可以注释掉或移除
    [key: string]: any;
  }

  export interface Status {
    property: string; // node-mpv v2 'status' 事件的结构
    value: any;
    // 保留旧的可能的属性，以防万一，但优先使用 property/value
    'time-pos'?: number;
    duration?: number;
    pause?: boolean;
    volume?: number;
  ...
  export interface EndFileEvent {
    reason: 'eof' | 'stop' | 'error' | 'quit' | 'unknown' | string; // jeffvli 的 fork 中 endfile 事件似乎不存在，但保留以防万一
  ...
  class MpvAPI {
    ...
    getDuration(): Promise<number | null>; // jeffvli 的 fork 返回 Promise<number>
    ...
    // on(event: 'endfile', listener: (event: EndFileEvent) => void): this; // jeffvli 的 fork 中不确定是否有此事件，MPV 本身有
